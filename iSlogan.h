...
DESCRIPTION: The iSlogan class has three encapsulated members, a user set string message
    that does not change and is inhereited by its' children, a random or user set value
    int p, and a private bool on which represents the object's state. If "On", iSlogan will 
    output its' message otherwise it will not output anything, however, no matter what state 
    iSlogan is, if p is the same value as a user queried int q, it will not output any message. 
    iSlogan's state is toggled on/off every pth request. 
...
public:
    ...
    */
    ...
    iSlogan(unsigned int input=1, std::string m ="default message");
    ...
    */
    ...
    std::string query(unsigned int q);
